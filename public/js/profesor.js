...
$(document).ready(function () {
  ...
  function populate_materie() {
    $.ajax({
      ...
      success: function (res) {
        ...
        res = JSON.parse(res);
        for (var i = 0; i < res.length; i++) {
          var materie = `<span class="nume_materie">Materie predata: ${res[i].materie}</span>`;

          $(".myusername-container").append(materie);
  ...
  function populate_studenti() {
    $.ajax({
      url: "/populate_studenti",
      type: "POST",
      dataType: 'text',
      data: {},
      success: function (res) {
        ...
        for (var i = 0; i < res.length; i++) {
          nota_materie = ""
          for (var j = 0; j < res[i].materie.length; j++) {

            if (Object.keys(res[i].materie[j])[0] == materie_global) {
              console.log(res[i].materie[j])
              nota_materie = res[i].materie[j][materie_global]
            }


          }
          var student = `<tr><td>${index}</td><td contenteditable="false">${res[i].prenume + " " + res[i].nume}<td contenteditable="false" class="user_student">${res[i].username}</td><td contenteditable="false">${res[i].grupa}</td><td contenteditable="true" class="note_student">${nota_materie}</td></tr>`;

          $(".tabeleditabil").append(student);
          index = index + 1
        }
      }
    });
  ...
  $(".buton_save_note").on("click", function () {


    var table = document.querySelector('.table.tabeleditabil tbody');
    ...
    for (var i = 1; i < rows.length; i++) {
      ...
      var userStudent = row.querySelector('.user_student').textContent;
      var noteStudent = row.querySelector('.note_student').textContent;


      var data = {
        user: userStudent,
        note: noteStudent.split(',').map(Number)
      };
    ...
    for (var j = 0; j < json.length; j++) {
      ...
      $.ajax({
        ...
        type: "POST",
        dataType: 'text',
        data: { note: json[j], materie_prof: materie_global},
        success: function (res) {
  ...
  var modal = document.getElementById("myModal");
    var btn = document.getElementById("open-modal2");
    ...
    btn.onclick = function() {
        ...
        $.ajax({
            url: '/ruta-catre-server',
            type: 'POST',
            data: JSON.stringify({ user_name: user_name }),
            contentType: 'application/json; charset=utf-8',
            success: function(result) {
                var tests = result.teste;
                $("#table-body").empty();
                tests.forEach(function(test) {
                    var tr = $("<tr/>");
                    tr.append($("<td/>", { text: test.nume_student }))
                      .append($("<td/>", { text: test.raspunsuri_corecte.join(', ') }))
                      .append($("<td/>", { text: test.raspunsuri_bifate.join(', ') }))
                      .append($("<td/>", { text: test.nota }));
    ...
    $('.download').click(function(e) {
      ...
      window.location.href = 'grading_template.png';
    });
